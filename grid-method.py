...
#Uniform distribution to create a logarithmically uniform distribution for z
lower_bound = 1e-5  
upper_bound = 5
num_points = 10

y = np.logspace(np.log10(lower_bound), np.log10(upper_bound), num=num_points)


# Define the number of samples taken
num_samples = 1000
...
def compute_power_spectrum(H0, ombh2, omch2, logA, ns):
    ...
    pars.set_matter_power(redshifts=(y), kmax=10.0)

    #Calculate the power spectrum from the parameters given and appending sequential z's onto the end of the first power spectrum
    pars.NonLinear = model.NonLinear_none
    results = camb.get_results(pars)
    kh, z, pk_2D = results.get_matter_power_spectrum(minkh=1e-4, maxkh=10, npoints = 200)
    pk = pk_2D.flatten()
...
for i, params in enumerate(sampled_params):
    H0, ombh2, omch2, logA, ns = params["H0"], params["ombh2"], params["omch2"], params["logA"], params["ns"]
    pk = compute_power_spectrum(H0, ombh2, omch2, logA, ns)
...
# Save the data
np.save(f'{output_dir}/grid_method_pk_data_{task_id}.npy', spectra_fp32)
np.save(f'{output_dir}/grid_method_params_data_{task_id}.npy', param_dict)
