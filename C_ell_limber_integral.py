...
# -------------------------------------------------------------------
def cl_limber_jax(l, allpars, zmin=1., zmax=5., nz=50, predicted_grid=None):
    # Create a redshift grid.
    z_vals = jnp.logspace(jnp.log10(zmin), jnp.log10(zmax), nz)
    ...
    W_vals = weight_function_jax(z_vals)
    ...
    integrand = (H_vals * (1. + z_vals)**2 / chi_vals**2) * (W_vals**2) * Pk_vals
...
l_vals_jax = jnp.logspace(jnp.log10(2.), jnp.log10(10000.), 1000)
...
plt.xlabel(r'$\ell$')
plt.ylabel(r'$C_\ell$')
plt.title(r'$C_\ell$ via Limber Approximation')
plt.legend()
plt.tight_layout()
plt.savefig('/home/jam249/rds/dlproject/C_ell_Plot')