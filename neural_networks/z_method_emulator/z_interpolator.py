...
predicted_test_Y = []  # This will hold the predictions for all test samples.
run_time = []          # To record runtime per test sample.

for i in range(num_iterations):
    # Build a dictionary with the i-th element for each key.
    current_pdict = {key: [pdict[key][i]] for key in pdict.keys()}
    
    # This will store the predictions (one per value in x) for the current test sample.
    sub_predictions = []
    
    start_time = time.perf_counter()
    
    for value in x:
        # Update the key 'z' with the current value from x.
        current_pdict['z'] = [value]
        
        # Get prediction from your model using predictions_flax.
        prediction = predictions_flax(current_pdict)
        
        # If the prediction comes as a 2D array of shape (1, 200), extract the 1D array.
        if hasattr(prediction, "ndim") and prediction.ndim == 2:
            prediction = prediction[0]
        
        # Append the prediction (now of shape (200,)) to the list.
        sub_predictions.append(prediction)
    
    end_time = time.perf_counter()
    run_time.append(end_time - start_time)
    
    # Append the 2D predictions for the current sample.
    predicted_test_Y.append(sub_predictions)

# Convert the list of predictions to a NumPy array.
# The resulting shape will be (num_iterations, len(x), 200)
predicted_test_Y = np.array(predicted_test_Y)

# Compute the average and standard deviation of runtime per test sample.
